	...
	FileList.prototype = {
		...
		_createRow: function(fileData, options) {
			...
			if (typeof(fileData.size) !== 'undefined' && fileData.size >= 0) {
				simpleSize = OC.Util.humanFileSize(parseInt(fileData.size, 10), true).replace("MB","مگابایت").replace("KB","کیلوبایت");
			...
			var text;
			if (mtime > 0) {
				formatted = new Intl.DateTimeFormat("fa").format(mtime);
				text = new Intl.DateTimeFormat("fa").format(mtime);
			} else {
				...
				return self.filesClient.move(dir + fileName, targetPath + fileName)
					.done(function() {
						...
						if (OC.joinPaths(self.getCurrentDirectory(), '/') === OC.joinPaths(dir, '/')) {
							...
							var changedtopersianlocale = OC.Util.humanFileSize(newSize);
							var finaltopersianlocale= changedtopersianlocale.replace("MB","مگابایت").replace("KB","کیلوبایت");
							oldFile.find('td.filesize').text(finaltopersianlocale);
		...
		updateSelectionSummary: function() {
			...
			if (summary.totalFiles === 0 && summary.totalDirs === 0) {
				...
				var changedtopersianlocale = OC.Util.humanFileSize(summary.totalSize);
				var finaltopersianlocale= changedtopersianlocale.replace("MB","مگابایت").replace("KB","کیلوبایت");
				this.$el.find('#headerSize a>span:first').text(finaltopersianlocale);
